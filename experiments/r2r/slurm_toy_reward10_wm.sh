...
#SBATCH --job-name=r2r-reward10-wm
#SBATCH --output=experiments/r2r/toy-reward10-wm-%j.out
...
OUTPUT=${RESULT_ROOT}/toy_acquisition/${CAMPAIGN}/distance8_reward10_world_model
REPLAY_DIRECTORY=${SLURM_TMPDIR:?}/r2r-reward10-wm-${CAMPAIGN}
...
apptainer exec --cleanenv --nv \
  ...
  python recall2imagine/train.py \
    ...
    --run.steps 50000 \
    --toy_arm reward10_world_model \
    --logdir "${OUTPUT}" \
    --replay_dir "${REPLAY_DIRECTORY}" \
    --lfs_dir "${OUTPUT}/lfs" \
    --use_lfs True \
    --wdb_name "R2R-toy-reward10-world-model" &
child=$!
wait "${child}"
status=$?
set -e
...
if summary['environment_steps'] != 50000:
  raise SystemExit(
      f"reward-10 arm ended at {summary['environment_steps']} rather than 50000")
if summary['learner_updates'] != 11476:
  raise SystemExit(
      f"reward-10 arm used {summary['learner_updates']} rather than 11476 updates")
PY
