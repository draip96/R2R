...
#SBATCH --array=0-1
#SBATCH --job-name=r2r-term-weight
#SBATCH --output=experiments/r2r/toy-terminal-weight-%A_%a.out
...
OUTPUT=${RESULT_ROOT}/toy_acquisition/${CAMPAIGN}/distance8_terminal_weight${TERMINAL_WEIGHT}_world_model
REPLAY_DIRECTORY=${SLURM_TMPDIR:?}/r2r-terminal-weight${TERMINAL_WEIGHT}-${CAMPAIGN}
...
apptainer exec --cleanenv --nv \
  ...
  python recall2imagine/train.py \
    ...
    --run.steps 50000 \
    --toy_arm "terminal_weight${TERMINAL_WEIGHT}_world_model" \
    --logdir "${OUTPUT}" \
    --replay_dir "${REPLAY_DIRECTORY}" \
    --lfs_dir "${OUTPUT}/lfs" \
    --use_lfs True \
    --wdb_name "R2R-toy-terminal-weight${TERMINAL_WEIGHT}-world-model" &
child=$!
wait "${child}"
status=$?
set -e
...
if summary['environment_steps'] != 50000:
  raise SystemExit(
      f"terminal-weight arm ended at {summary['environment_steps']} rather than 50000")
if summary['learner_updates'] != 11476:
  raise SystemExit(
      f"terminal-weight arm used {summary['learner_updates']} rather than 11476 updates")
