...
#SBATCH --array=0-1
#SBATCH --job-name=r2r-cont-promote
#SBATCH --output=experiments/r2r/toy-cont-promotion-%A_%a.out
...
OUTPUT=${RESULT_ROOT}/toy_cont_promotion/${CAMPAIGN}/${ARM}
REPLAY_DIRECTORY=${SLURM_TMPDIR:?}/r2r-cont-promotion-${CAMPAIGN}-${ARM}
...
apptainer exec --cleanenv --nv \
  ...
  python recall2imagine/train.py \
    --configs "toy_memory,toy_distance8,r2r_w64,${ARM_CONFIG},toy_balanced_terminal_cont_memory" \
    --seed 0 \
    --run.steps 50000 \
    --toy_arm "balanced_terminal_cont_${ARM}" \
    --logdir "${OUTPUT}" \
    --replay_dir "${REPLAY_DIRECTORY}" \
    --lfs_dir "${OUTPUT}/lfs" \
    --use_lfs True \
    --wdb_name "R2R-toy-balanced-cont-${ARM}" &
child=$!
wait "${child}"
status=$?
set -e
...
if summary['environment_steps'] != 50000:
  raise SystemExit(
      f"continuation promotion ended at {summary['environment_steps']} rather than 50000")
if summary['learner_updates'] != 11476:
  raise SystemExit(
      f"continuation promotion used {summary['learner_updates']} rather than 11476 updates")
