...
#SBATCH --job-name=r2r-r2i100k
#SBATCH --output=experiments/r2r/toy-r2i-long-%j.out
...
OUTPUT=${RESULT_ROOT}/toy_controls/${SOURCE_CAMPAIGN}/r2i_w1024
REPLAY_DIRECTORY=${SLURM_TMPDIR:?}/r2r-toy-r2i-${SOURCE_CAMPAIGN}-${TARGET_STEPS}
PROVENANCE=${OUTPUT}/continuations/${TARGET_STEPS}/provenance
...
apptainer exec --cleanenv --nv \
  ...
  python recall2imagine/train.py \
    --configs "toy_memory,toy_distance8,r2r_w1024,toy_r2i_reference" \
    --seed 0 \
    ...
    --lfs_dir "${OUTPUT}/lfs" \
    --use_lfs True \
    --wdb_name "R2R-toy-controls-r2i-w1024-${TARGET_STEPS}" &
child=$!
wait "${child}"
status=$?
set -e
