...
#SBATCH --array=0-1
#SBATCH --job-name=r2r-cont-audit
#SBATCH --output=experiments/r2r/toy-continuation-audit-%A_%a.out
...
OUTPUT=${RESULT_ROOT}/toy_continuation_audit/${CAMPAIGN}/${ARM}
REPLAY_DIRECTORY=${SLURM_TMPDIR:?}/r2r-cont-${CAMPAIGN}-${ARM}
...
if summary['environment_steps'] != 25000:
  raise SystemExit(
      f"continuation arm ended at {summary['environment_steps']} rather than 25000")
if summary['learner_updates'] != 5226:
  raise SystemExit(
      f"continuation arm used {summary['learner_updates']} rather than 5226 updates")
