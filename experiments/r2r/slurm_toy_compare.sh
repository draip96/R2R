...
#SBATCH --array=0-2
#SBATCH --job-name=r2r-toy-compare
#SBATCH --output=experiments/r2r/toy-compare-%A_%a.out
...
case "${SLURM_ARRAY_TASK_ID}" in
  0)
    ARM=world_model_only
    ARM_CONFIG=toy_world_model_only
    ;;
  1)
    ARM=direct_bptt
...
OUTPUT=${RESULT_ROOT}/toy_compare/${CAMPAIGN}/${ARM}
REPLAY_DIRECTORY=${SLURM_TMPDIR:?}/r2r-toy-compare-${CAMPAIGN}-${ARM}
...
apptainer exec --cleanenv --nv \
  ...
  python recall2imagine/train.py \
    --configs "toy_memory,toy_distance8,r2r_w64,${ARM_CONFIG}" \
    --seed 0 \
    --logdir "${OUTPUT}" \
    --replay_dir "${REPLAY_DIRECTORY}" \
    --lfs_dir "${OUTPUT}/lfs" \
    --use_lfs True \
    --wdb_name "R2R-toy-compare-${ARM}" &
child=$!
wait "${child}"
status=$?
set -e
...
if summary['environment_steps'] != 25000:
  raise SystemExit(
      f"matched arm ended at {summary['environment_steps']} rather than 25000")
PY
