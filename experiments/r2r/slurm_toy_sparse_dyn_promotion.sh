...
#SBATCH --array=0-1
#SBATCH --job-name=r2r-sparse-promote
#SBATCH --output=experiments/r2r/toy-sparse-promotion-%A_%a.out
...
if not math.isfinite(value) or not 0.0 < value < 0.05:
  raise SystemExit('R2R_DYN_SCALE must be finite and strictly between 0 and 0.05')
PY

DYN_TAG=${DYN_SCALE//./p}
OUTPUT=${RESULT_ROOT}/toy_sparse_dyn_promotion/${CAMPAIGN}/dyn${DYN_TAG}_${ARM}
REPLAY_DIRECTORY=${SLURM_TMPDIR:?}/r2r-sparse-promotion-${CAMPAIGN}-${ARM}
...
printf 'arm=%s\ndynamics_loss_scale=%s\n' "${ARM}" "${DYN_SCALE}" \
  > "${OUTPUT}/provenance/promotion.txt"
...
apptainer exec --cleanenv --nv \
  ...
  python recall2imagine/train.py \
    ...
    --run.steps 50000 \
    --toy_arm "balanced_sparse_dyn${DYN_TAG}_${ARM}" \
    --logdir "${OUTPUT}" \
    --replay_dir "${REPLAY_DIRECTORY}" \
    --lfs_dir "${OUTPUT}/lfs" \
    --use_lfs True \
    --wdb_name "R2R-toy-balanced-sparse-dyn${DYN_TAG}-${ARM}" &
child=$!
wait "${child}"
status=$?
set -e
...
if summary['environment_steps'] != 50000:
  raise SystemExit(
      f"sparse-dynamics promotion ended at {summary['environment_steps']} rather than 50000")
if summary['learner_updates'] != 11476:
  raise SystemExit(
      f"sparse-dynamics promotion used {summary['learner_updates']} rather than 11476 updates")
