...
#SBATCH --array=0-1
#SBATCH --job-name=r2r-sparse-prior
#SBATCH --output=experiments/r2r/toy-sparse-prior-%A_%a.out
...
OUTPUT=${RESULT_ROOT}/toy_sparse_prior_promotion/${CAMPAIGN}/${ARM}
REPLAY_DIRECTORY=${SLURM_TMPDIR:?}/r2r-sparse-prior-${CAMPAIGN}-${ARM}
...
apptainer exec --cleanenv --nv \
  ...
  python recall2imagine/train.py \
    ...
    --seed 0 \
    --run.steps 50000 \
    --toy_arm "balanced_sparse_dyn_cont_${ARM}" \
    --logdir "${OUTPUT}" \
    --replay_dir "${REPLAY_DIRECTORY}" \
    --lfs_dir "${OUTPUT}/lfs" \
    --use_lfs True \
    --wdb_name "R2R-toy-balanced-sparse-prior-${ARM}" &
child=$!
wait "${child}"
status=$?
set -e
...
if summary['environment_steps'] != 50000:
  raise SystemExit(
      f"sparse-prior promotion ended at {summary['environment_steps']} rather than 50000")
if summary['learner_updates'] != 11476:
  raise SystemExit(
      f"sparse-prior promotion used {summary['learner_updates']} rather than 11476 updates")
